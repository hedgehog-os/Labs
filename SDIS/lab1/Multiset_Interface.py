...
def main():
    ...
    while True:
        ...
        elif parts[0] == 'show' and parts[1] in multisets:
        ...
        elif parts[0] == 'delete' and parts[1] in multisets and parts[2] in multisets[parts[1]].multiset:
            multisets[parts[1]].delete(parts[2])

        elif parts[0] == 'ndelete' and parts[1] in multisets and ':' in parts[2]:
            elem, num = parts[2].split(':', 1)
            multisets[parts[1]].ndelete(elem, int(num))
        
        elif parts[0] == 'isempty' and parts[1] in multisets:
            print(f"{parts[1]} is empty: {multisets[parts[1]].is_empty()}")

        elif parts[0] == 'cardinality':
            print(f"Cardinality of {parts[1]}: {multisets[parts[1]].cardinality()}")

        elif parts[0] == 'rm':
            del multisets[parts[1]]

        elif parts[1] == '+' and parts[0] in multisets and parts[2] in multisets:
        ...
        elif parts[0] == 'bolean' and parts[1] in multisets:
            result = multisets[parts[1]].bolean()
            print(f"Булеан множества {parts[1]} содержит {len(result)} подмножеств:")
